...
// Each step reads its own title + description (its sections keep their own audio
// buttons inside ArticleRenderer), so steps without sections can still be heard.
function stepId(step) { return `g${route.params.id}-s${step.id}-step` }
