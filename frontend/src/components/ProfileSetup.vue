...
const questions = computed(() => [
  ...
  },
  {
    ...
    type: 'boolean',
    required: false,
    shortLabel: t('profile.timeline.hasDrivingLicense'),
    label: t('profile.hasDrivingLicense.label'),
  },
  {
    id: 'computerSkills',
    type: 'choice',
    required: false,
    shortLabel: t('profile.timeline.computerSkills'),
    label: t('profile.computerSkills.label'),
    sublabel: t('profile.computerSkills.sublabel'),
    options: [
      { value: 'none', label: t('profile.computerSkills.none') },
      { value: 'basic', label: t('profile.computerSkills.basic') },
      { value: 'advanced', label: t('profile.computerSkills.advanced') },
    ],
  },
  {
    id: 'educationLevel',
    type: 'choice',
    required: false,
    shortLabel: t('profile.timeline.educationLevel'),
          ...
          <div v-for="(q, i) in questions" :key="q.id" class="flex items-start gap-3">
            <div class="flex flex-col items-center shrink-0">
              <button
                ...
                :class="getStepVisualState(i) === 'current'
                  ? 'bg-brand-600 border-brand-600'
                  : getStepVisualState(i) === 'answered' ? 'bg-surface-600 border-surface-600' : 'bg-white border-surface-300'"
                @click="goToStep(i)"
              >
                <CheckIcon v-if="getStepVisualState(i) === 'answered'" class="w-3.5 h-3.5 text-white" />
                <QuestionMarkCircleIcon v-else-if="getStepVisualState(i) === 'skipped'" class="w-4 h-4 text-surface-400" />
            ...
            <template v-for="(q, i) in questions" :key="q.id">
              <button
                ...
                :class="getStepVisualState(i) === 'current'
                  ...
                  : getStepVisualState(i) === 'answered' ? 'bg-surface-600 border-surface-600 p-1.5'
                  : getStepVisualState(i) === 'skipped' ? 'bg-white border-white p-1.5'
                  : 'bg-surface-600 border-surface-400 p-1.5'"
                @click="goToStep(i)"
              >
                <CheckIcon v-if="getStepVisualState(i) === 'answered'" class="w-3.5 h-3.5 text-white shrink-0" />
                <QuestionMarkCircleIcon v-else-if="getStepVisualState(i) === 'skipped'" class="w-4 h-4 text-surface-500 shrink-0" />
